        ...
        <Toolbar disableGutters>
          ...
          {logged && <Box sx={{ flexGrow: 1, display: { xs: 'none', md: 'flex' } }}>
            {pages.map((page) => (
              <Link
                // key={page}
                to={page === "Home" ? "/" : (page === "My List" ? "/list" : "/")}
                // onClick={handleCloseNavMenu}
                style={{textDecoration:"none", display: "flex", flexDirection:"row", gap:"10px", padding:"18px", color: "white"}}
                // href={page === "Home" ? "/" : (page === "My List" ? "/list" : "/")}
              >
                {page}
              </Link>
            ))}
